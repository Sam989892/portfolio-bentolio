...
export default function Bentolio({
  ...
  return (
    <div className="relative flex flex-col gap-4 mx-auto sm:px-0 p-3 sm:p-6 w-full max-w-[1200px]" style={{ color: "#1d1d1f" }}>
      <AnimatePresence>
        <header key="header" className="rounded-[20px] w-full">
          ...
          >
            <h1 className="mb-4 sm:mb-0 font-light text-xl sm:text-2xl" style={{ color: "#1d1d1f" }}>
              <i>{name.first}</i>{" "}
          ...
          <div className="gap-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 lg:col-span-6">
            <div className="sm:col-span-2 lg:col-span-4 rounded-[20px] h-full">
              ...
              >
                ...
                <p className="m-0 w-full sm:w-[90%] font-bold text-3xl sm:text-[44px] leading-tight sm:leading-[44px]" style={{ color: "#1d1d1f" }}>
                  {title.split(curvedText).map((part, i) => (
                    <React.Fragment key={`title-part-${i}`}>
                      {i === 0 ? (
                        part
                      ) : (
                        <React.Fragment key={`curved-text-${i}`}>
              ...
              >
                {projects.map((project, index) => (
                  <div key={project.name}>
                    {index === 0 ? (
                      <React.Fragment key={`project-first-${project.name}`}>
                        <Link
                          href={project.link || "#"}
                          style={{
                            textDecoration: "none",
